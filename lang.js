...
const showAnim = anim => {
  ...
  for(time = 0; time <= anim.duration; time += 0.2) {
    s += showColor(anim.color(time))
  }
  return('[' + s + ']')
}

const showStack = stack => {
  console.log('stack:', stack.map(showAnim).join('  '))
}

const evalWords = (stack0, program) => {
  let stack = stack0
  showStack(stack)
  for(w of program) {
    console.log('#', w)
...
evalWords([], ['black', 'white',  'fade', 'white', 'black', 'fade', 'join'])
